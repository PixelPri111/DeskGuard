...
export default function DeskTile({ desk }) {
  ...
  function handleClick() {
    ...
    if (desk.status === 'abandoned' && isMyDesk) setShowStillHere(true);
  }
  ...
  return (
    <>
      <div
        onClick={handleClick}
        className={`
          relative border-2 rounded-xl p-3 w-28 h-28 flex flex-col justify-between
          transition-all duration-500 select-none
          ${STATUS_STYLES[desk.status]}
          ${desk.status === 'occupied' ? 'animate-pulse-slow' : ''}
        `}
      >
